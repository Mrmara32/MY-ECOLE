...
def _calculer_montant(personnel, mois):
    ...
    if personnel['type_remuneration'] == 'horaire':
        h_seances = db.execute(
            "SELECT COALESCE(SUM(duree_heures),0) as h FROM seances_cours WHERE personnel_id=? AND substr(date_seance,1,7)=? AND statut='validee'",
            (personnel['id'], mois)
        ).fetchone()['h']
        source = 'seances'
        h = h_seances
        if h_seances == 0:
            h_manuel = db.execute(
                "SELECT COALESCE(SUM(nombre_heures),0) as h FROM heures_enseignement WHERE personnel_id=? AND mois=?",
                (personnel['id'], mois)
            ).fetchone()['h']
            if h_manuel > 0:
                h = h_manuel
...
def _calculer_prime_revision(personnel_id, mois):
    ...
    total_heures_mois = db.execute(
        "SELECT COALESCE(SUM(duree_heures),0) as h FROM revision_seances WHERE strftime('%Y-%m',date_seance)=?", (mois,)
    ).fetchone()['h']
    ...
    mes_heures = db.execute(
        """SELECT COALESCE(SUM(duree_heures),0) as h FROM revision_seances
           WHERE personnel_id=? AND strftime('%Y-%m',date_seance)=? AND redistribue=0""",
        (personnel_id, mois)
    ).fetchone()['h']
    if mes_heures <= 0:
        return {'prime': 0, 'heures': 0}
...
def _reference_salaire(personnel_id):
    ...
    dernier = db.execute(
        "SELECT montant_net FROM bulletins_salaire WHERE personnel_id=? ORDER BY mois DESC LIMIT 1", (personnel_id,)
    ).fetchone()
...
def calculer(personnel_id):
    ...
    prime_rev = _calculer_prime_revision(personnel_id, mois)
    calc['prime_revision'] = prime_rev['prime']
    calc['heures_revision'] = prime_rev['heures']

    avance = db.execute(
        "SELECT * FROM avances_salaire WHERE personnel_id=? AND mois_remboursement=? AND statut='en_cours'",
        (personnel_id, mois)
    ).fetchone()
    calc['avance_en_attente'] = row_to_dict(avance) if avance else None

    deja_paye = db.execute(
        "SELECT * FROM bulletins_salaire WHERE personnel_id=? AND mois=?", (personnel_id, mois)
    ).fetchone()
...
def liste_paie():
    ...
    mois = request.args.get('mois') or datetime.now().strftime('%Y-%m')
    personnel = db.execute("SELECT * FROM personnel ORDER BY nom, prenom").fetchall()
    result = []
    for p in personnel:
        calc = _calculer_montant(dict(p), mois)
        prime_rev = _calculer_prime_revision(p['id'], mois)
        calc['prime_revision'] = prime_rev['prime']
        bulletin = db.execute(
            "SELECT * FROM bulletins_salaire WHERE personnel_id=? AND mois=?", (p['id'], mois)
        ).fetchone()
        row = dict(p)
        row.update(calc)
...
def soumettre_validation():
    ...
    existing = db.execute("SELECT * FROM validations_paie WHERE mois=?", (mois,)).fetchone()
    ...
    for p in personnel:
        calc = _calculer_montant(dict(p), mois)
        prime = _calculer_prime_revision(p['id'], mois)
    ...
    if existing:
        ...
        db.execute(
            "INSERT INTO validations_paie (id,mois,statut,masse_salariale_totale,soumis_par) VALUES (?,?,?,?,?)",
            (vid, mois, 'attente_directeur', masse, g.user['id']),
        )
    db.commit()
...
def valider_directeur(v_id):
    body = request.get_json(silent=True) or {}
    v = db.execute("SELECT * FROM validations_paie WHERE id=?", (v_id,)).fetchone()
    if not v:
        return jsonify({'error': 'Introuvable'}), 404
...
def create_type_prime():
    ...
    if db.execute("SELECT id FROM types_primes WHERE nom=?", (nom,)).fetchone():
        return jsonify({'error': 'Ce type de prime existe déjà'}), 409
    tid = gen_id('tp')
    db.execute("INSERT INTO types_primes (id,nom) VALUES (?,?)", (tid, nom))
    db.commit()
...
def update_type_prime(t_id):
    body = request.get_json(silent=True) or {}
    db.execute(
        "UPDATE types_primes SET nom=COALESCE(?,nom), active=COALESCE(?,active) WHERE id=?",
        (body.get('nom'), (1 if body.get('active') else 0) if 'active' in body else None, t_id),
    )
    db.commit()
    row = db.execute("SELECT * FROM types_primes WHERE id=?", (t_id,)).fetchone()
    return jsonify(row_to_dict(row))
...
def delete_type_prime(t_id):
    db.execute("DELETE FROM types_primes WHERE id=?", (t_id,))
    db.commit()
...
def list_avances():
    ...
    sql = """SELECT a.*, p.nom, p.prenom, u.full_name as cree_par_nom
             ...
             LEFT JOIN users u ON u.id=a.cree_par WHERE 1=1"""
    params = []
...
def plafond_avance(personnel_id):
    ...
    ref = _reference_salaire(personnel_id)
    if ref is None:
    ...
    en_cours = db.execute(
        "SELECT COALESCE(SUM(montant),0) as s FROM avances_salaire WHERE personnel_id=? AND statut='en_cours'",
        (personnel_id,)
    ).fetchone()['s']
...
def create_avance():
    ...
    db.execute(
        "INSERT INTO avances_salaire (id,personnel_id,montant,motif,date_avance,mois_remboursement,cree_par) "
        "VALUES (?,?,?,?,?,?,?)",
        (aid, personnel_id, montant, body.get('motif'), date_avance, mois_remboursement, g.user['id']),
    )
    ...
    db.execute(
        ...
        "VALUES (?,?,?,?,?,?,?,?,?,?)",
        (tid, 'sortie', date_avance, f"Avance sur salaire — {p['prenom']} {p['nom']}",
...
def annuler_avance(a_id):
    a = db.execute("SELECT * FROM avances_salaire WHERE id=?", (a_id,)).fetchone()
    if not a:
        return jsonify({'error': 'Introuvable'}), 404
    if a['statut'] != 'en_cours':
        return jsonify({'error': 'Seule une avance en cours peut être annulée'}), 400
    db.execute("UPDATE avances_salaire SET statut='annulee' WHERE id=?", (a_id,))
    db.commit()
    return jsonify({'success': True})
...
def generer_bulletin():
    ...
    if g.user['role'] == 'comptable':
        validation = db.execute("SELECT * FROM validations_paie WHERE mois=?", (mois,)).fetchone()
    ...
    if db.execute("SELECT id FROM bulletins_salaire WHERE personnel_id=? AND mois=?", (personnel_id, mois)).fetchone():
    ...
    db.execute(
        """INSERT INTO bulletins_salaire
           (id,personnel_id,mois,type_remuneration,heures,taux_horaire,salaire_base,prime_revision,
            heures_revision,primes,primes_detail,deductions,avance_deduite,montant_net,date_paiement,genere_par)
           VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
        (bid, personnel_id, mois, calc['type_remuneration'], calc['heures'], calc['taux_horaire'],
    ...
    db.execute(
        "INSERT INTO transactions (id,type,date_op,description,categorie,moyen_paiement,montant,reference,cree_par,statut_validation) "
        "VALUES (?,?,?,?,?,?,?,?,?,?)",
        (tid, 'sortie', date_paiement, f"Salaire {mois} — {p['prenom']} {p['nom']}",
...
def list_bulletins():
    ...
    sql = """SELECT b.*, p.nom, p.prenom, p.poste, p.matiere FROM bulletins_salaire b
             JOIN personnel p ON p.id=b.personnel_id WHERE 1=1"""
    params = []
...
def get_bulletin(b_id):
    row = db.execute(
        """SELECT b.*, p.nom, p.prenom, p.poste, p.matiere, p.photo_url, p.cycle_enseignement
           FROM bulletins_salaire b JOIN personnel p ON p.id=b.personnel_id WHERE b.id=?""", (b_id,)
    ).fetchone()
    if not row:
        return jsonify({'error': 'Introuvable'}), 404
    return jsonify(row_to_dict(row))
...
def delete_bulletin(b_id):
    b = db.execute("SELECT * FROM bulletins_salaire WHERE id=?", (b_id,)).fetchone()
    if not b:
        return jsonify({'error': 'Introuvable'}), 404
    db.execute("DELETE FROM bulletins_salaire WHERE id=?", (b_id,))
    db.commit()
